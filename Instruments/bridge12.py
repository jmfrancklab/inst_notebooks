    ...
    def set_power(self,dBm):
        ...
        if setting > 400:
            ...
            if setting > 30+self.cur_pwr_int: 
                raise RuntimeError("Once you are above 10 dBm, you must raise the power in MAX 3 dB increments.  The power is currently %g, and you tried to set it to %g -- this is not allowed!\nNote that if you are running from the power control server, you shouldn't encounter this error!"%(self.cur_pwr_int/10.,setting/10.))
        self.write(b'power %d\r'%setting)
        self.read_until(b"Power updated\r\n")

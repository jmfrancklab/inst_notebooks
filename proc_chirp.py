...
d = nddata_hdf5('180102_chirp_LCs.h5/capture1_180102',
            directory=getDATADIR(exp_type='test_equip'))
with figlist_var(filename='chirp_180102.pdf') as fl:
    ...
    fl.next('check for phase roll')
    fl.plot(abs(d['ch',1]/d['ch',0]), label='ratio')
    fl.plot((d['ch',1]/d['ch',0]).angle/2./pi, '.', label='phase diff.')
    fl.plot((d['ch',1]/d['ch',0]*exp(-1j*pi)).angle/pi, '.', label='subtracted-phase diff.')

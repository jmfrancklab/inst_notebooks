...
for date,id_string,numchan in [
    ...
    for k in xrange(ndshape(onephase)['ph1']):
        for j in xrange(ndshape(onephase)['repeat']):
            ...
            fl.plot(abs(onephase['repeat',j]['t':(25.6e-6,26.1e-6)]['ph1',k].C.reorder('t',first=True)),color=colors[k],alpha=0.3)
            fl.next('compare falling edge')
            # need to define time slice for falling edge
            fl.plot(abs(onephase['repeat',j]['t':(26.8e-6,28.0e-6)]['ph1',k].C.reorder('t',first=True)),color=colors[k],alpha=0.3)
            fl.next('compare rising edge, raw')
            fl.plot((onephase_raw['repeat',j]['t':(25.6e-6,26.1e-6)]['ph1',k].C.reorder('t',first=True)),color=colors[k],alpha=0.3)
            fl.next('compare falling edge, raw')
            fl.plot((onephase_raw['repeat',j]['t':(26.8e-6,28.0e-6)]['ph1',k].C.reorder('t',first=True)),color=colors[k],alpha=0.3)
    ...
    pulse_slice = s_raw['t':(25.6e-6,27.5e-6)]['ch',1].real
    ...
    fl.image(s_raw)
    #}}}
    pulse_slice = s_raw['t':(-1.0e-6,1.0e-6)]['ch',1].real
    ...
    for k in xrange(ndshape(onephase_raw)['ph1']):
        for j in xrange(ndshape(onephase_raw)['repeat']):
            ...
            fl.plot((onephase_raw_shift['repeat',j]['ph1',k]['t':(-1e-6,-0.1e-6)].C.reorder('t',first=True)+2*k),color=colors[k],alpha=0.3)
            fl.next('compare falling edge: uncorrected')
            fl.plot((onephase_raw_shift['repeat',j]['ph1',k]['t':(0.1e-6,1e-6)].C.reorder('t',first=True)+2*k),color=colors[k],alpha=0.3)
    ...
    for k in xrange(ndshape(onephase_rawc)['ph1']):
        for j in xrange(ndshape(onephase_rawc)['repeat']):
            ...
            fl.plot(onephase_rawc['repeat',j]['ph1',k]['t':(-1e-6,-0.1e-6)].C.reorder('t',first=True),color=colors[k],alpha=0.3)
            fl.next('compare falling edge: corrected')
            fl.plot(onephase_rawc['repeat',j]['ph1',k]['t':(0.1e-6,1e-6)].C.reorder('t',first=True),color=colors[k],alpha=0.3)
    ...
    measured_phase = analytic['t':(-0.6e-6,0.6e-6)].mean('t',return_error=False).mean('ph2',return_error=True).mean('average',return_error=True)
    ...
    s_coherence_domain.mean('average')
    fl.next('again')
    fl.image(s_coherence_domain)
    signal = s_coherence_domain['ph1',1]['ph2',0]

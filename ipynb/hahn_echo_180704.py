...
t90 = 2.1e-6
omega1_nominal = 2*pi/(4*t90)
results = ndshape([4,4,100,1000],['ph2','ph1','omega1','Omega']).alloc()
omega1_axis = linspace(0,3*omega1_nominal,ndshape(results)['omega1'])
omega_axis = linspace(-omega1_nominal*10,omega1_nominal*10,ndshape(results)['Omega'])
...
for ph2 in xrange(0,4,2):
    for ph1 in xrange(4):
        for i,omega1 in enumerate(omega1_axis):
            for j,Omega in enumerate(omega_axis):
                pul_rot = lambda I,ph: expm(-1j*pi/2*Iz*ph).dot(I).dot(1j*pi/2*Iz*ph)
...
fl.next('filter for second pulse',figsize=(14,14)) # I want figsize, and I need fl so that it handles the units
figure(figsize=(14,14))
fl.image(results['ph2',2])

...
def acquire():
    ...
    try_again = True
    while try_again:
        data_name = 'capture%d_180130'%j
        data.name(data_name)
        try:
            data.hdf5_write('180130_1amp.h5')
            try_again = False
...
def gen_pulse(freq=15e6, width=4e-6, ch1_only=True):
    with AFG() as a:
        ...
        for this_ch in range(1):
            ...
            raw_input("Manually set AFG to low amplitude, then turn on RF amp.")
            set_amp = raw_input('What amplitude in Vpp? ')
            set_amp = float(set_amp)
